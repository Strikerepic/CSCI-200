...
void LinkedList<T>::sort() {
    ...
    LinkedList<T> leftArr;
    LinkedList<T> rightArr;

    int midPoint = size()/2;
    //Get LA
    for(int i = 0; i < midPoint; i++) {
        leftArr.insert(i,get(i));
    }

    for(int i = midPoint; i < size(); i++) {
        rightArr.insert(i-midPoint,get(i));
    }
    ...
    //Now the merge part FUCK!

    LinkedList<T> mergedArray;


    int leftArrayCount = 0;
    int rightArrayCount = 0;
    int mergeArrayCount = 0;
    


    while (leftArrayCount < leftArr.size() && rightArrayCount < rightArr.size()) {
    // Compare and merge elements
        if (leftArr.get(leftArrayCount) <= rightArr.get(rightArrayCount)) {
            mergedArray.insert(mergeArrayCount, leftArr.get(leftArrayCount));
            leftArrayCount++;
        } else {
            mergedArray.insert(mergeArrayCount, rightArr.get(rightArrayCount));
            rightArrayCount++;
        }
        mergeArrayCount++;
    ...
    while (leftArrayCount < leftArr.size()) {
        mergedArray.insert(mergeArrayCount, leftArr.get(leftArrayCount));
        leftArrayCount++;
        mergeArrayCount++;
    }
    ...
    while (rightArrayCount < rightArr.size()) {
        mergedArray.insert(mergeArrayCount, rightArr.get(rightArrayCount));
        rightArrayCount++;
        mergeArrayCount++;
    }
...
template<typename T>
int LinkedList<T>::search(T targ) {
    int val = this->find(targ);
    return val;
}
